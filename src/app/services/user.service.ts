...
export class UserService {
  private userApi = 'http://apicine.herokuapp.com/public/api/user';
 ...
 getLogin(email: string, password: string): Observable<any> {
    const postUrl = 'http://apicine.herokuapp.com/public/api/login';
...
changeInformation(id: number, name: string, address: string, phone: string, password: string): Observable<any> {
    const putUrl = 'http://apicine.herokuapp.com/public/api/user/' + id;
    const headers = new Headers({'Content-Type': 'application/json'});
...
signUp(newAccount: any): Observable<any> {
  const url = 'http://apicine.herokuapp.com/public/api/register';
  const headers = new Headers({'Content-Type': 'application/json'});
